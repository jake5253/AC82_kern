/*
 *  linux/arch/arm/mach-MT85XX/mt85xx_CS_watch.c
 *
...
static MT85XX_CS_WATCH_T _t_cs_watch;
...
void mt85xx_CS_watch_init(void)
{
...
void mt85xx_CS_watch_start(uint32_t u4Flag)
{
	if(_t_cs_watch._fg_CS_watch_on)
	{
		if((u4Flag & PSR_IRQ_DISABLE) != PSR_IRQ_DISABLE) 
		{
			...
			{
				printk("[mt85xx_CS_watch_start] Fail.\n");
			}
		}	
	}
}

void mt85xx_CS_watch_end(uint32_t u4Flag)
{
...
void volatile mt85xx_CS_watch_dump(void)
{
	uint32_t regSP;
...
EXPORT_SYMBOL(mt85xx_CS_watch_start);
EXPORT_SYMBOL(mt85xx_CS_watch_end);
EXPORT_SYMBOL(mt85xx_CS_watch_dump);
#endif // CONFIG_MT85XX_CS_WATCH
