/*
 *  linux/arch/arm/mach-MT85XX/mt85xx_isr_watch.c
 *
...
// Global variable
//==================================================
MT85XX_ISR_OVERHEAD_T ar_isr_overhead_vector[NR_IRQS];


//==================================================
// Private functions
//==================================================
void _mt85xx_isrw_reset(void)
{
...
void mt85xx_isr_watch_init(void)
{
	_mt85xx_isrw_reset();
}

void mt85xx_isrw_proc_init(void)
{
...
void mt85xx_isr_watch_trigger(uint32_t u4Vector, uint32_t event)
{
	T64b_RAW_TIME_T rt_delta;
	HAL_TIME_T      t_delta;
	MT85XX_ISR_OVERHEAD_T *pr_isr_over_head;
	...
	switch(event)
	{
		case ISRW_EVT_START:
			...
			{
				printk(KERN_ERR "[mt85xx_isr_watch_trigger] ERROR: get start raw time fail.\n");
				return;
			}
			break;
		case ISRW_EVT_END:
			...
			{
				printk(KERN_ERR "[mt85xx_isr_watch_trigger] ERROR: get end raw time fail.\n");
				return;
			}
			...
			{
				printk(KERN_ERR "[mt85xx_isr_watch_trigger] ERROR: start < end.\n");
				return;
			}
			...
			{
				printk(KERN_ERR "[mt85xx_isr_watch_trigger] ERROR: get end time fail.\n");
				return;
			}
			...
			{
				printk(KERN_ERR "[mt85xx_isr_watch_trigger] ERROR: raw to time fail.\n");
				return;
			}
			...
			if(i_hal_t64b_add_time(	&(pr_isr_over_head->rTotalTime),
									...
									&t_delta) != 0)
			{
				printk(KERN_ERR "[mt85xx_isr_watch_trigger] ERROR: add time fail.\n");
				return;
			}
